...
angular
  ...
  .filter('extractClPublicId', extractClPublicId)
  ...
  return {
    link: function(scope, element) {
      var listener = function(event, toState, toParams, fromState, fromParams) {
        // Default title
        var title = 'Duqhan | Backend';
        // Create your own title pattern
        if (toState.data && toState.data.pageTitle) title = 'Duqhan | ' + toState.data.pageTitle;
        $timeout(function() {
          element.text(title);
        });
      };
...
function extractClPublicId() {
  return function(input) {
    var a = input.lastIndexOf("/");
    var b = input.lastIndexOf(".");
    var publicId = input.substring(a + 1, b);
    return publicId;
  };
}